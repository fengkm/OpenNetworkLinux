...
static char qsfpdd_nif_port_status_cpld_addr_array[QSFPDD_NIF_PORT_NUM][5] = { "0030", "0030", "0030", "0030", "0030", \
...
static char qsfpdd_fab_port_status_cpld_addr_array[QSFPDD_FAB_PORT_NUM][5] = { "0031", "0031", "0031", "0032", "0032", \
...
/* to get cpld qsfpdd nif port status register */
static int ufi_get_qsfpdd_nif_port_status_reg(int port_id, int *reg)
{
    int ret = ONLP_STATUS_OK;
    int cpld_qsfpdd_nif_port_status_reg = 0;
    char qsfpdd_nif_port_status_sysfs[255] = "";
    char buf[ONLP_CONFIG_INFO_STR_MAX] = "";
    int bus_id = 1;
    int len = 0;

    snprintf(qsfpdd_nif_port_status_sysfs, sizeof(qsfpdd_nif_port_status_sysfs),
            "/sys/bus/i2c/devices/%d-%s/cpld_qsfpdd_nif_port_status_%s",
            bus_id,
            qsfpdd_nif_port_status_cpld_addr_array[port_id],
            qsfpdd_nif_port_status_cpld_index_array[port_id]);

    ONLP_TRY(onlp_file_read((uint8_t*)&buf, ONLP_CONFIG_INFO_STR_MAX, &len, qsfpdd_nif_port_status_sysfs));

    /* qsfpdd_nif_port_status_sysfs output is hex value (0xab), hex to int */
    cpld_qsfpdd_nif_port_status_reg = (int) strtol((char *)buf, NULL, 0);

    *reg = cpld_qsfpdd_nif_port_status_reg;

    return ret;
}

/* to get cpld qsfpdd nif port config register */
static int ufi_get_qsfpdd_nif_port_config_reg(int port_id, int *reg)
{
    int ret = ONLP_STATUS_OK;
    int cpld_qsfpdd_nif_port_config_reg = 0;
    char qsfpdd_nif_port_config_sysfs[255] = "";
    char buf[ONLP_CONFIG_INFO_STR_MAX] = "";
    int bus_id = 1;
    int len = 0;

    snprintf(qsfpdd_nif_port_config_sysfs, sizeof(qsfpdd_nif_port_config_sysfs),
            "/sys/bus/i2c/devices/%d-%s/cpld_qsfpdd_nif_port_config_%s",
            bus_id,
            qsfpdd_nif_port_status_cpld_addr_array[port_id],
            qsfpdd_nif_port_status_cpld_index_array[port_id]);

    ONLP_TRY(onlp_file_read((uint8_t*)&buf, ONLP_CONFIG_INFO_STR_MAX, &len, qsfpdd_nif_port_config_sysfs));

    /* qsfpdd_nif_port_config_sysfs output is hex value (0xab), hex to int */
    cpld_qsfpdd_nif_port_config_reg = (int) strtol((char *)buf, NULL, 0);

    *reg = cpld_qsfpdd_nif_port_config_reg;

    return ret;
}

/* to set cpld qsfpdd nif port config register */
static int ufi_set_qsfpdd_nif_port_config_reg(int port_id, int value)
{
    int ret = ONLP_STATUS_OK;
    char qsfpdd_nif_port_config_sysfs[255] = "";
    int bus_id = 1;
    ...
    ONLP_TRY(onlp_file_write_int(value, qsfpdd_nif_port_config_sysfs));

    return ret;
}

/* to get cpld qsfpdd fabric port status register */
static int ufi_get_qsfpdd_fab_port_status_reg(int port_id, int *reg)
{
    int ret = ONLP_STATUS_OK;
    int cpld_qsfpdd_fab_port_status_reg = 0;
    char qsfpdd_fab_port_status_sysfs[255] = "";
    char buf[ONLP_CONFIG_INFO_STR_MAX] = "";
    int bus_id = 1;
    int len = 0;

    snprintf(qsfpdd_fab_port_status_sysfs, sizeof(qsfpdd_fab_port_status_sysfs),
            "/sys/bus/i2c/devices/%d-%s/cpld_qsfpdd_fab_port_status_%s",
            bus_id,
            qsfpdd_fab_port_status_cpld_addr_array[port_id],
            qsfpdd_fab_port_status_cpld_index_array[port_id]);

    ONLP_TRY(onlp_file_read((uint8_t*)&buf, ONLP_CONFIG_INFO_STR_MAX, &len, qsfpdd_fab_port_status_sysfs));

    /* qsfpdd_fab_port_status_sysfs output is hex value (0xab), hex to int */
    cpld_qsfpdd_fab_port_status_reg = (int) strtol((char *)buf, NULL, 0);

    *reg = cpld_qsfpdd_fab_port_status_reg;

    return ret;
}

/* to get cpld qsfpdd fabric port config register */
static int ufi_get_qsfpdd_fab_port_config_reg(int port_id, int *reg)
{
    int ret = ONLP_STATUS_OK;
    int cpld_qsfpdd_fab_port_config_reg = 0;
    char qsfpdd_fab_port_config_sysfs[255] = "";
    char buf[ONLP_CONFIG_INFO_STR_MAX] = "";
    int bus_id = 1;
    int len = 0;

    snprintf(qsfpdd_fab_port_config_sysfs, sizeof(qsfpdd_fab_port_config_sysfs),
            "/sys/bus/i2c/devices/%d-%s/cpld_qsfpdd_fab_port_config_%s",
            bus_id,
            qsfpdd_fab_port_status_cpld_addr_array[port_id],
            qsfpdd_fab_port_status_cpld_index_array[port_id]);

    ONLP_TRY(onlp_file_read((uint8_t*)&buf, ONLP_CONFIG_INFO_STR_MAX, &len, qsfpdd_fab_port_config_sysfs));

    /* qsfpdd_fab_port_config_sysfs output is hex value (0xab), hex to int */
    cpld_qsfpdd_fab_port_config_reg = (int) strtol((char *)buf, NULL, 0);

    *reg = cpld_qsfpdd_fab_port_config_reg;

    return ret;
}

/* to set cpld qsfpdd fab port config register */
static int ufi_set_qsfpdd_fab_port_config_reg(int port_id, int value)
{
    ...
    ONLP_TRY(onlp_file_write_int(value, qsfpdd_fab_port_config_sysfs));
...
static int ufi_get_port_present_status(int local_id, int *status)
{
    ...
    //QSFPDD NIF, QSFPDD Fabric, SFP Ports
    if (local_id < QSFPDD_NIF_PORT_NUM) { //QSFPDD NIF
        port_id = local_id;
        ret = ufi_get_qsfpdd_nif_port_status_reg(port_id, &port_status_reg);
        if (ret != ONLP_STATUS_OK) {
            AIM_LOG_ERROR("ufi_get_qsfpdd_nif_port_status_reg() failed, port=%d\n", port_id);
            return ONLP_STATUS_E_INTERNAL;
        } else {
            *status = !((port_status_reg & 0b00000010) >> 1);
        }
    } else if (local_id >= QSFPDD_NIF_PORT_NUM && local_id < (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM)) { //QSFPDD Fabric
    ...
    } else if (local_id >= (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM) && local_id < ALL_PORT_NUM) { //SFP
                       ...
                       uint8_t* dst, int len)
{
    ...
    if (local_id < QSFPDD_NIF_PORT_NUM) {
        /* QSFPDD NIF */
        port_id = local_id;
    ...
    } else if (local_id >= QSFPDD_NIF_PORT_NUM && local_id < (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM)) {
        /* QSFPDD Fabric */
        port_id = local_id - QSFPDD_NIF_PORT_NUM;
        bus_id = qsfpdd_fab_port_eeprom_bus_id_array[port_id];
        ONLP_TRY(onlp_i2c_block_read(bus_id, devaddr, addr, len, dst, ONLP_I2C_F_FORCE));

    } else if (local_id >= (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM) && local_id < ALL_PORT_NUM) {
        /* SFP */
        port_id = local_id - (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM);
    ...
    switch (control) {
        case ONLP_SFP_CONTROL_RESET:
            if (port_type == ONLP_SFP_TYPE_QSFP28) {
                ...
                if (local_id < QSFPDD_NIF_PORT_NUM) { //QSFPDD NIF
                    port_id = local_id;
                    ret = ufi_get_qsfpdd_nif_port_config_reg(port_id, &port_config_reg);
                    if (ret != ONLP_STATUS_OK) {
                        AIM_LOG_ERROR("ufi_get_qsfpdd_nif_port_config_reg() failed, port=%d\n", port_id);
                        return ret;
                    } else {
                        if (value == 0) {
                        ...
                        /* set value into qsfpdd nif port config register */
                        ret = ufi_set_qsfpdd_nif_port_config_reg(port_id, port_config_reg);
                        if (ret != ONLP_STATUS_OK) {
                            AIM_LOG_ERROR("ufi_set_qsfpdd_nif_port_config_reg() failed, port=%d, value=%d\n", port_id, port_config_reg);
                            return ret;
                        }
                    }
                } else if (local_id >= QSFPDD_NIF_PORT_NUM && local_id < (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM)) { //QSFPDD Fabric
                    ...
                    if (ret != ONLP_STATUS_OK) {
                        ...
                        /* set value into qsfpdd fab port config register */
        ...
        case ONLP_SFP_CONTROL_TX_DISABLE:
            if (port_type == ONLP_SFP_TYPE_SFP) {
                ...
                if (ret != ONLP_STATUS_OK) {
                    ...
                    if (value == 0) {
                        if (port_id == (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM)) { //SFP Port0
                            port_config_reg = port_config_reg & 0b11111110;
                        } else if (port_id == (QSFPDD_NIF_PORT_NUM + QSFPDD_FAB_PORT_NUM + 1)) { //SFP Port1
