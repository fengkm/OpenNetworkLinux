...
static int update_ledi_system_info(int local_id, onlp_led_info_t* info)
{
    ...
    ret = onlp_i2c_modifyb(50, 0x75, 0x02, 0xFE, 0x01, ONLP_I2C_F_FORCE);
...
static int update_ledi_psu_info(int local_id, onlp_led_info_t* info)
{
    ...
    if (local_id == ONLP_LED_PSU1) {
        if (psu_presence == 1 && psu_pwgood == 1) {
            /* LED GREEN */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111110, 0b00000001, ONLP_I2C_F_FORCE);
            ret |= onlp_i2c_modifyb(50, 0x75, 0x02, 0b11110111, 0b00000000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111110, 0b00000001, ONLP_I2C_F_FORCE);
            ret |= onlp_i2c_modifyb(50, 0x75, 0x02, 0b11110111, 0b00001000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
            ...
            /* LED OFF */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111110, 0b00000000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
        ...
        if (psu_presence == 1 && psu_pwgood == 1) {
            /* LED GREEN */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111101, 0b00000010, ONLP_I2C_F_FORCE);
            ret |= onlp_i2c_modifyb(50, 0x75, 0x02, 0b11101111, 0b00000000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111101, 0b00000010, ONLP_I2C_F_FORCE);
            ret |= onlp_i2c_modifyb(50, 0x75, 0x02, 0b11101111, 0b00010000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_ORANGE;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED OFF */
            ret  = onlp_i2c_modifyb(50, 0x75, 0x03, 0b11111101, 0b00000000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_OFF;
            } else {
                return ONLP_STATUS_E_INTERNAL;
...
static int update_ledi_fan_info(int local_id, onlp_led_info_t* info)
{
    ...
    if (fan_absent_flag == 1) {
        /* LED OFF */
        ret = onlp_i2c_modifyb(50, 0x75, 0x02, 0b11111001, 0b00000000, ONLP_I2C_F_FORCE);
        if (ret == ONLP_STATUS_OK) {
            info->mode = ONLP_LED_MODE_OFF;
        } else {
            return ONLP_STATUS_E_INTERNAL;
        ...
        /* LED GREEN */
        ret = onlp_i2c_modifyb(50, 0x75, 0x02, 0b11111001, 0b00000010, ONLP_I2C_F_FORCE);
        if (ret == ONLP_STATUS_OK) {
            info->mode = ONLP_LED_MODE_GREEN;
        } else {
            return ONLP_STATUS_E_INTERNAL;
        ...
        /* LED ORANGE */
        ret = onlp_i2c_modifyb(50, 0x75, 0x02, 0b11111001, 0b00000110, ONLP_I2C_F_FORCE);
        if (ret == ONLP_STATUS_OK) {
            info->mode = ONLP_LED_MODE_ORANGE;
        } else {
            return ONLP_STATUS_E_INTERNAL;
...
static int update_ledi_fantray_info(int local_id, onlp_led_info_t* info)
{
    ...
    if (local_id == ONLP_LED_FAN_TRAY1) {
        ...
        if (fan_presence == 0) {
            ...
            /* LED GREEN */
            ret = onlp_i2c_modifyb(59, 0x20, 0x03, 0b11001111, 0b00100000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret = onlp_i2c_modifyb(59, 0x20, 0x03, 0b11001111, 0b00010000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_ORANGE;
            } else {
                return ONLP_STATUS_E_INTERNAL;
        ...
        if (fan_presence == 0) {
            ...
            /* LED GREEN */
            ret = onlp_i2c_modifyb(59, 0x20, 0x03, 0b11111100, 0b00000010, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret = onlp_i2c_modifyb(59, 0x20, 0x03, 0b11111100, 0b00000001, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_ORANGE;
            } else {
                return ONLP_STATUS_E_INTERNAL;
        ...
        if (fan_presence == 0) {
            ...
            /* LED GREEN */
            ret = onlp_i2c_modifyb(59, 0x20, 0x02, 0b11001111, 0b00100000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret = onlp_i2c_modifyb(59, 0x20, 0x02, 0b11001111, 0b00010000, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_ORANGE;
            } else {
                return ONLP_STATUS_E_INTERNAL;
        ...
        if (fan_presence == 0) {
            ...
            /* LED GREEN */
            ret = onlp_i2c_modifyb(59, 0x20, 0x02, 0b11111100, 0b00000010, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_GREEN;
            } else {
                return ONLP_STATUS_E_INTERNAL;
            ...
            /* LED ORANGE */
            ret = onlp_i2c_modifyb(59, 0x20, 0x02, 0b11111100, 0b00000001, ONLP_I2C_F_FORCE);
            if (ret == ONLP_STATUS_OK) {
                info->mode = ONLP_LED_MODE_ORANGE;
            } else {
                return ONLP_STATUS_E_INTERNAL;
